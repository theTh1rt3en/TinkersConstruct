...
public class TinkerFMP {
    ...
    public void init(FMLInitializationEvent event) {
        ...
        if (TinkerWorld.metalBlock != null) {
            // metal blocks
            for (int i = 0; i < 11; i++) BlockMicroMaterial.createAndRegister(TinkerWorld.metalBlock, i);
        }
        ...
        if (TinkerSmeltery.smeltery != null) {
            ...
            for (int i = 2; i < 12; i++) {
                ...
                BlockMicroMaterial.createAndRegister(
                        ...
                        i,
                        TinkerSmeltery.smelteryNether.getUnlocalizedName() + "Nether");
            }

            // brownstone
            for (int i = 0; i < 7; i++) BlockMicroMaterial.createAndRegister(TinkerSmeltery.speedBlock, i);

            // clear glass
            BlockMicroMaterial.createAndRegister(TinkerSmeltery.clearGlass, 0);
            for (int i = 0; i < 16; i++) BlockMicroMaterial.createAndRegister(TinkerSmeltery.stainedGlassClear, i);
        }

        if (TinkerTools.multiBrick != null) {
            // chisel bricks
            for (int i = 0; i < 14; i++) BlockMicroMaterial.createAndRegister(TinkerTools.multiBrick, i);

            for (int i = 0; i < 16; i++) BlockMicroMaterial.createAndRegister(TinkerTools.multiBrickFancy, i);
        }
    }
}
