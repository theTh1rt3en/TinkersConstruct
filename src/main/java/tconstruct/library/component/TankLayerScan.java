...
public class TankLayerScan extends LogicComponent {
    ...
    // Recursion causes overflows on 32-bit, so reduce if not 64-bit
    private static final int MAX_LAYER_RECURSION_DEPTH = System.getProperty("os.arch").equals("amd64") ? 4000 : 2000;
    ...
    public CoordTuple getAirByIndex(int index) {
        if (index >= airCoords.size() || index < 0) return null;

        return airCoords.get(index);
    }

    private byte getDirection() {
        if (master instanceof IFacingLogic) return ((IFacingLogic) master).getRenderDirection();
        return 0;
    }
    ...
    protected boolean checkServant(int x, int y, int z) {
        Block block = world.getBlock(x, y, z);
        if (block == null || world.isAirBlock(x, y, z) || !isValidBlock(x, y, z)) return false;
    ...
    protected boolean initialRecurseLayer(int x, int y, int z) {
        if (bricks > MAX_LAYER_RECURSION_DEPTH) return false;

        CoordTuple keystone = new CoordTuple(x, y, z);
        if (keystone.equals(returnStone)) return true;
    ...
    protected boolean floodTest(int x, int y, int z) {
        if (airBlocks > MAX_LAYER_RECURSION_DEPTH) return false;
    ...
    public int recurseStructureDown(int y) {
        ...
        if (i.hasNext()) {
            ...
            if (checkAir(coord.x, y, coord.z)) {
                ...
                if (valid) return recurseStructureDown(y - 1);
    ...
    public int recurseStructureUp(int y) {
        ...
        if (i.hasNext()) {
            ...
            if (checkServant(coord.x, y, coord.z)) {
                ...
                if (valid) recurseStructureUp(y + 1);
            }
        }
        return y - 1;
    }
    ...
    public void recheckStructure() {
        ...
        if (height != -1) {
            if (height <= master.yCoord) invalidateStructure();
            else invalidateBlocksAbove(height);
        } else {
            if (structureTop == 0) // Workaround for missing data
            {
                for (CoordTuple coord : blockCoords) structureTop = coord.y;
            }
