...
public abstract class ProjectileBase extends EntityArrow implements IEntityAdditionalSpawnData {
    ...
    protected void playHitBlockSound(int x, int y, int z) {
        ...
        if (block != null) worldObj.playSoundAtEntity(this, block.stepSound.getBreakSound(), 0.7f, 1.0f);
    }
    ...
    public void onHitEntity(MovingObjectPosition movingobjectposition) {
        ...
        for (ActiveToolMod toolmod : TConstructRegistry.activeModifiers) {
                ...
                if (dmg > baseDamage) baseDamage = dmg;
            }
        damage += baseDamage;
        ...
        // unlike the regular weapons, we do no gain more damage from potion of strength/weakness since we don't hit
        // stuff directly
        ...
        if (!dealDamage(damage, ammo, tags, movingobjectposition.entityHit)) {
            if (!bounceOnNoDamage) this.setDead();
        ...
        // we hit it, BURNNNNNN
        else AbilityHelper.processFiery(shootingEntity, movingobjectposition.entityHit, tags);
        ...
        if (!(movingobjectposition.entityHit instanceof EntityEnderman)) {
            ...
            if (rand.nextInt(10) + 1 > tags.getInteger("Reinforced")) this.setDead();
            else {
    ...
    public void onUpdate() {
        ...
        // luckily we can call this directly and take the arrow-code, since we'd have to call super.onUpdate otherwise.
        ...
        if (this.inGround) updateInGround();
        else updateInAir();
    }
        ...
        if (!this.worldObj.isRemote && this.inGround && this.arrowShake <= 0) {
            ...
            if (this.canBePickedUp == 1) {
                ...
                if (this.returnStack != null && this.returnStack.getItem() instanceof AmmoItem) {
                    if (!((AmmoItem) this.returnStack.getItem()).pickupAmmo(returnStack, null, player)) flag = false;
                }
                // regular pickup.
                // Note that you can't pick up ammo items without the proper itemstack in your inventory to prevent
                // dupes
