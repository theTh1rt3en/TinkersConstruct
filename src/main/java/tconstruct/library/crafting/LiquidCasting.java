...
public class LiquidCasting {
    ...
    private boolean contains(CastingRecipe recipe) {
        for (CastingRecipe c : casts) {
            if (c.matches(recipe.castingMetal, recipe.cast)) return true;
        }
        return false;
    }
     ...
     */
    public void addCastingRecipe(
            ItemStack output, FluidStack metal, ItemStack cast, int delay, FluidRenderProperties props) {
