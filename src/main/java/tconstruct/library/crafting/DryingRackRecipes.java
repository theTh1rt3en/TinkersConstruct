...
public class DryingRackRecipes {
    ...
    public static void addDryingRecipe(Object input, int time, Object output) {
        ...
        else if (input instanceof Block) inputItem = new ItemStack((Block) input, 1, 0);
        else throw new RuntimeException("Drying recipe input is invalid!");

        if (output instanceof ItemStack) outputItem = (ItemStack) output;
        else if (output instanceof Item) outputItem = new ItemStack((Item) output, 1, 0);
        else if (output instanceof Block) outputItem = new ItemStack((Block) output, 1, 0);
        else throw new RuntimeException("Drying recipe output is invalid!");
    ...
    public static int getDryingTime(ItemStack input) {
        for (DryingRecipe r : recipes) {
            if (r.matches(input)) return r.time;
        }
    ...
    public static class DryingRecipe {
        ...
        public boolean matches(ItemStack input) {
            ...
            if (input.hasTagCompound()) {
                ...
                if (input.getTagCompound().hasNoTags()) input.setTagCompound(null);
            }
