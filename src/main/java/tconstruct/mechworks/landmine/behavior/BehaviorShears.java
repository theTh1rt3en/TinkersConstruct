...
public class BehaviorShears extends Behavior {
    ...
    public void executeLogic(World par1World, int par2, int par3, int par4, ItemStack par5ItemStack, Entity triggerer,
        ...
        List<IShearable> sheeps = par1World.getEntitiesWithinAABB(
                IShearable.class,
                AxisAlignedBB.getBoundingBox(par2 - 2, par3 - 2, par4 - 2, par2 + 2, par3 + 2, par4 + 2));
        ...
        if (sheeps != null && !sheeps.isEmpty()) {
            ...
            while (i1.hasNext()) {
                ...
                if (sheep.isShearable(par5ItemStack, par1World, par2, par3, par4)) {
                    if (rand.nextInt(2) == 0) {
                        ArrayList<ItemStack> drops = sheep.onSheared(
                                par5ItemStack,
                                par1World,
                                par2,
                                par3,
                                par4,
                                EnchantmentHelper.getEnchantmentLevel(Enchantment.fortune.effectId, par5ItemStack));
