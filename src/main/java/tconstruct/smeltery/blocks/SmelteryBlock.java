...
public class SmelteryBlock extends InventoryBlock {
    ...
    public IIcon getIcon(IBlockAccess world, int x, int y, int z, int side) {
        ...
        if (meta == 1) // Drain
        {
            if (side == direction) return icons[5];
            else if (side / 2 == direction / 2) return icons[4];
            else return icons[3];
        } else if (meta == 2) {
            return icons[6];
        } else if (meta == 11) {
    ...
    public void onBlockPlacedBy(World world, int x, int y, int z, EntityLivingBase entityliving, ItemStack stack) {
        ...
        if (world.getBlockMetadata(x, y, z) == 0) onBlockPlacedElsewhere(world, x, y, z, entityliving);
    }
