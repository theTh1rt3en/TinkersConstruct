...
public class TActiveOmniMod extends ActiveToolMod {
    ...
    private boolean autoSmelt(ToolCore tool, NBTTagCompound tags, ItemStack stack, int x, int y, int z,
        ...
        if (block.getMaterial().isToolNotRequired()) {
            ...
            if (tool instanceof HarvestTool) {
                if (!((HarvestTool) tool).isEffective(block, blockMeta)) return false;
            } else return false;
        } else if (!ForgeHooks.canToolHarvestBlock(block, blockMeta, stack)) return false;
    ...
    private void baconator(ToolCore tool, ItemStack stack, EntityLivingBase entity, NBTTagCompound tags) {
        ...
        if (random.nextInt(chance) < bacon) {
            ...
            else AbilityHelper.spawnItemAtEntity(entity, new ItemStack(TinkerWorld.strangeFood, 1, 2), 0);
        }
    }
    ...
    private void slimify(ToolCore tool, NBTTagCompound tags, Block block, int x, int y, int z, World world) {
        ...
        // regular slime
        if (tags.getInteger("Head") == slimeMat) count++;
        if (tags.getInteger("Handle") == slimeMat) count++;
        if (tags.getInteger("Accessory") == slimeMat) count++;
        if (tags.getInteger("Extra") == slimeMat) count++;
