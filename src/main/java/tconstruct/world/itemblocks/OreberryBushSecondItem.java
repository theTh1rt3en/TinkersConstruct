...
public class OreberryBushSecondItem extends MultiItemBlock {
    ...
    public boolean onItemUse(ItemStack stack, EntityPlayer player, World world, int x, int y, int z, int side,
        ...
        if (meta % 4 != 1 && world.getFullBlockLightValue(x, y + 1, z) >= 13) return false;
        else if (player.canPlayerEdit(x, y, z, side, stack) && player.canPlayerEdit(x, y + 1, z, side, stack)) {
            Block block = world.getBlock(x, y, z);

            if (block != null && block.canSustainPlant(world, x, y, z, ForgeDirection.UP, TinkerWorld.oreBerrySecond)
